...
//function enemySelector(){
   // var randoEnemy = monsters[Math.floor(Math.random() * monsters.length)]
...
function attack(randomEnemy) {
    while (playersLife > 0 && randomEnemy.hp > 0) {
        
        let playerAttacks = Math.floor(Math.random() * (randomEnemy.maxDamage - randomEnemy.minDamage)) + randomEnemy.minDamage;
        let enemyAttacks = Math.floor(Math.random() * (randomEnemy.maxDamage - randomEnemy.minDamage));
        let battle = readlineSync.keyIn("You attack or you run, choose: [a]ttack or [r]un?", { limit: ["a", "r"] })
        if (battle === "a") {
            console.log("Attack...")
            randomEnemy.HP = randomEnemy.HP - playerAttacks
            console.log(playerAttacks, enemyAttacks, randomEnemy.HP)
            console.log(`${user}!, ${randomEnemy.enemyName} is hurt! ${randomEnemy.enemyName} took ${playerAttacks} points of damage! ${randomEnemy.enemyName} currently has ${randomEnemy.HP} health points. `)
            
            playersLife = playersLife - enemyAttacks
            console.log(`${randomEnemy.enemyName} is attacking you! ${randomEnemy.enemyName} dealt ${enemyAttacks} points of damage to you. You have ${playersLife} health points left.`)
            
            if (randomEnemy.HP < 0) {
                inventory.push(randomEnemy.item)
                console.log(`${user}, you defeated ${randomEnemy.enemyName}. You have a received an item. Check inventory...`)
                console.log(inventory)
                console.log("Continue walking...")
                enemySelector()
                console.log(monsters)
                monsters=monsters.splice(randomEnemy, 1)[0]
